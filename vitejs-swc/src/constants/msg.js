...
export const MSG_DELETED_SUCCESSFULLY = (obj, f) => `Delete ${obj} id:'${f}' is successfully`;
export const MSG_OBJ_UPDATE_SUCCESSFULLY = (obj) => `Update ${obj} successfully`;

// Fields
export const MSG_FIELD_REQUIRED = (f) => `Field ${f} is required`;
export const MSG_FIELD_MUST_URL = (f) => `Field :${f} must be url`;
export const MSG_FIELD_MUST_ALPHANUMERIC = (f) => `Field :${f} must be alphanumeric`;
export const MSG_FIELD_MUST_NUMBER = (f) => `Field :${f} must be number`;
...
export const MSG_CREATED_NEWS = (name) => `${name} have just created a new piece of news`;
export const MSG_CREATED_MENTION_NEWS = (name) => `${name} has just mentioned you in a news article`;